...
class DomBertEmbeddings(nn.Module):
    ...
    """
    def __init__(self, config):
        super(DomBertEmbeddings, self).__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)
        # TODO: add path to embdding file to config
        wv = gensim.models.KeyedVectors.load_word2vec_format('/Users/ds/git/malware-text/model/vector/malware_GoogleNews_300d_comb.txt')
...
class DomBertModel(BertPreTrainedModel):
    ...
    """
    def __init__(self, config):
        super(DomBertModel, self).__init__(config)
        self.config = config
...
class DomBertForSequenceClassification(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
