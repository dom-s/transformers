...
class DomBertEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        if self.lambda_mode == 'var':
            self._lambda = nn.Linear(2 * self.word_embeddings.embedding_dim, 1)
        else:
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, domain_input_ids=None):
        ...
        elif self.lambda_mode == 'var':
            domain_embeddings = self.W(domain_embeddings)
            _lambda = torch.sigmoid(self._lambda(torch.cat((inputs_embeds, domain_embeddings), dim=2)))
            rectified_embeddings = (1 - _lambda) * inputs_embeds + _lambda * domain_embeddings
        else:
