...
from .configuration_dombert import DomBertConfig
...
class DomBertEmbeddingsVarLambda(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
    ...
    def init_domain_embeddings(self, domain_embedding_path):
        ...
        self.W = nn.Linear(self.domain_embeddings.embedding_dim, self.word_embeddings.embedding_dim,
                           bias=False)
        self._lambda = nn.Linear(self.domain_embeddings.embedding_dim + self.word_embeddings.embedding_dim, 1)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, domain_input_ids=None):
        ...
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
        domain_embeddings = self.domain_embeddings(domain_input_ids)
        _lambda = self.sigmoid(self._lambda(torch.cat((inputs_embeds, domain_embeddings), dim=2)))
        rectified_embeddings = (1-_lambda) * inputs_embeds + _lambda * self.W(domain_embeddings)
...
class DomBertModel(DomBertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
        ...
        embedding_output = self.embeddings(input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, domain_input_ids=domain_input_ids)
...
class DomBertModelVarLambda(DomBertPreTrainedModel):
    r"""
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        **last_hidden_state**: ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, hidden_size)``
            Sequence of hidden-states at the output of the last layer of the model.
    ...
    def __init__(self, config):
        ...
        self.embeddings = DomBertEmbeddingsVarLambda(config)
        self.encoder = BertEncoder(config)
        self.pooler = BertPooler(config)
...
class DomBertForSequenceClassification(DomBertPreTrainedModel):
    r"""
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        outputs = self.bert(input_ids,
                            ...
                            domain_input_ids=domain_input_ids)
...
class DomBertVarLambdaForSequenceClassification(DomBertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
    ...
    def __init__(self, config):
        ...
        self.bert = DomBertModelVarLambda(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, self.config.num_labels)
...
class DomBertForTokenClassification(DomBertPreTrainedModel):
    r"""
...
class DomBertVarLambdaForTokenClassification(DomBertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            Labels for computing the token classification loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
